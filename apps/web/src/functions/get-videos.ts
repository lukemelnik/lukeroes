...
function getBestThumbnail(item: YouTubePlaylistItem): string | null {
  const thumbnails = item.snippet?.thumbnails && Object.values(item.snippet.thumbnails);
  if (!thumbnails || thumbnails.length === 0) return null;

  const preferredOrder = ["maxres", "standard", "high", "medium", "default"] as const;
...
function getVideoId(item: YouTubePlaylistItem): string | null {
  return item.snippet?.resourceId?.videoId ?? item.contentDetails?.videoId ?? null;
}
  ...
  return items
    .map((item) => {
      ...
      return {
        ...
        thumbnailUrl:
          getBestThumbnail(item) ?? `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`,
      } satisfies ApiVideo;
    })
