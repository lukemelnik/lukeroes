...
async function fetchReleaseDetails(releaseId: number): Promise<ReleaseResponse> {
  const apiUrl = process.env.SONGKEEPER_API_URL;
  const accessKey = process.env.SONGKEEPER_ACCESS_KEY;
...
export async function getReleaseDetailsById(releaseId: number): Promise<ReleaseDetail | undefined> {
