...
function VideosPageComponent() {
  const { data: videos, isLoading, isError, error, refetch, isRefetching } = useVideos();
  ...
  const otherVideos = useMemo(
    () => (videos && featuredVideo ? videos.filter((v) => v.id !== featuredVideo.id) : []),
        ...
        <Card className="w-full max-w-md p-6 text-center">
          <CardContent className="space-y-4 p-0">
            ...
            <p className="text-muted-foreground">
              {error instanceof Error ? error.message : "Could not load videos right now."}
            </p>
          ...
          <Card className="group cursor-pointer" onClick={() => setSelectedVideo(featuredVideo)}>
            <CardContent className="p-0">
              <div className="relative aspect-video">
                <img
                  src={featuredVideo.thumbnailUrl || getThumbnailUrl(featuredVideo.youtubeId)}
              ...
              <div className="p-6">
                <h2 className="mb-2 font-bold text-2xl">{featuredVideo.title}</h2>
              ...
              {otherVideos.map((video) => (
                ...
                >
                  <CardContent className="p-0">
                    <div className="relative aspect-video">
                      <img
                        src={video.thumbnailUrl || getThumbnailUrl(video.youtubeId)}
                        alt={video.title}
                    ...
                    <div className="p-4">
                      <h3 className="mb-1 line-clamp-2 font-semibold text-lg">{video.title}</h3>
                      {video.publishedAt && (
                        <p className="text-muted-foreground text-sm">
