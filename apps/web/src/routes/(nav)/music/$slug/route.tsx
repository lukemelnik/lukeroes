...
import { createFileRoute, notFound, Outlet, useNavigate, useParams } from "@tanstack/react-router";
...
export const Route = createFileRoute("/(nav)/music/$slug")({
  loader: async ({ params, context }) => {
    ...
    const releases = context.queryClient.getQueryData<ReleaseSummary[]>(musicQueryOptions.queryKey);
...
function ReleaseLayout() {
  ...
  const { data: release, isPending } = useQuery(releaseDetailsQueryOptions(match?.id ?? null));
              ...
              <CardContent className="flex flex-col items-center p-6">
                <div className="space-y-4 md:w-2/3">
                  ...
                  <div>
                    ...
                    <p className="mb-1 text-muted-foreground text-sm">{release.artist}</p>
                    <p className="text-muted-foreground text-sm">
                      {release.type} · {formatDate(release.releaseDate)}
                    </p>
                    {release.version && (
                      <p className="text-muted-foreground text-sm">Version: {release.version}</p>
                    )}
                  </div>
