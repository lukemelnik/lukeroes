...
function MusicPageComponent() {
  ...
  const selectedRelease =
    releases?.find((r) => (releaseParam ? slugify(r.title) === releaseParam : false)) ??
    releases?.[0];

  const selectedReleaseSlug = selectedRelease ? slugify(selectedRelease.title) : null;
      ...
      <div className="flex min-h-[60vh] w-full items-center justify-center px-4">
        <Card className="w-full max-w-md p-6 text-center">
          <CardContent className="space-y-4 p-0">
            ...
            <p className="text-muted-foreground">Check back soon for releases.</p>
          </CardContent>
        </Card>
      </div>
    );
            ...
            <Card className="sticky top-6">
              <CardContent className="flex flex-col items-center p-6">
                <div className="w-2/3 space-y-4">
                  ...
                  <div>
                    <h2 className="mb-1 font-bold text-2xl">{selectedRelease.title}</h2>
                    <p className="mb-1 text-muted-foreground text-sm">{selectedRelease.artist}</p>
                  ...
                  <div className="w-full space-y-1">
                    {tracksForSelected.length > 0 ? (
                      tracksForSelected.map((track) => (
                        <Link
                          key={track.id ?? `${track.trackNumber}-${track.title}`}
                          to="/music/$slug/$track"
                          params={{
                      ...
                      <p className="text-muted-foreground text-sm">Tracklist not available.</p>
                    )}
                  </div>
                </div>
              </CardContent>
                ...
                {releases.map((item) => (
                  <Card
                    ...
                    className={`group cursor-pointer overflow-hidden transition-all hover:shadow-lg ${
                      selectedReleaseSlug !== slugify(item.title) ? "opacity-80 blur-[0.5px]" : ""
                    }`}
                    ...
                    <CardContent className="flex flex-col items-center p-6">
                      <div className="w-2/3 space-y-2">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="truncate text-muted-foreground text-sm">{item.artist}</p>
                          <p className="text-muted-foreground text-sm">
                            {item.type} · {new Date(item.releaseDate).toLocaleDateString("en-US")}
                          </p>
                        </div>
                      </div>
                    </CardContent>
            ...
            <Card key={item.id} className="overflow-hidden">
              <CardContent className="p-0">
                ...
                <div
                  ...
                  tabIndex={0}
                  onClick={() => {
                    const newId = expandedMobileId === String(item.id) ? null : String(item.id);
                ...
                >
                  <div className="flex flex-col items-center p-4">
                    ...
                    <div className="mb-3 w-full text-center">
                      ...
                      <p className="mb-1 text-muted-foreground text-sm">{item.artist}</p>
                      <p className="text-muted-foreground text-sm">
                        {item.type} · {new Date(item.releaseDate).toLocaleDateString("en-US")}
                      </p>
                    </div>
