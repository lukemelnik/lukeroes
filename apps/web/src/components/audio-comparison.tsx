...
export function AudioComparison({
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      ...
      const maxDuration = Math.max(beforeAudio.duration || 0, afterAudio.duration || 0);
  ...
  const switchTrack = (track: "before" | "after") => {
    ...
    const currentPlayTime =
      activeTrack === "before" ? beforeAudio.currentTime : afterAudio.currentTime;
    ...
    <div className="rounded-xl border bg-card p-4 md:p-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className={cn(
            "flex size-12 flex-shrink-0 items-center justify-center rounded-full transition",
      ...
      <audio ref={beforeRef} src={beforeSrc} preload="metadata" className="hidden" />
      {/* biome-ignore lint/a11y/useMediaCaption: Audio comparison for quality, no speech to caption */}
      <audio ref={afterRef} src={afterSrc} preload="metadata" className="hidden" />
    </div>
  );
}
