...
import { getPastDates, getUpcomingDates, type TourDate, tourDates } from "@/lib/tour-config";
...
export default function TourSection({ showPastDates = false, limit }: TourSectionProps) {
  const upcomingDates = getUpcomingDates(tourDates);
  const pastDates = getPastDates(tourDates);

  const displayedUpcoming = limit ? upcomingDates.slice(0, limit) : upcomingDates;
    ...
    <section className="w-full px-4 py-16 md:px-6">
      <div className="container mx-auto max-w-4xl">
        <div className="mb-8 text-center">
          ...
          <p className="text-muted-foreground">Catch a live performance near you</p>
        </div>
        ...
        {showPastDates && pastDates.length > 0 && (
          <div className="mt-12">
            <h3 className="mb-4 font-semibold text-muted-foreground text-xl">Past Shows</h3>
